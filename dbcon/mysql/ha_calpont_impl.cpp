...
//                        (*f)->field_length = strlen(tmp);
                ...
                case CalpontSystemCatalog::TIMESTAMP:
                {
                    ...
                    intColVal = row.getUintField<8>(s);
/*
                    TimeStamp timestamp(intColVal);
                    ulong mysql_sec_part = (ulong) timestamp.msecond;
                    // It is assumed here that the value stored in timestamp.second
                    // can fit in my_time_t
                    my_time_t mysql_ts = (my_time_t) timestamp.second;
*/
...
/*                    Field_timestamp* f2 = (Field_timestamp*)*f;
                    f2->store_timestamp(mysql_ts, mysql_sec_part);
*/
                    Field_varstring* f2 = (Field_varstring*)*f;
                    f2->store(tmp, strlen(tmp), f2->charset());
                    break;
