...
/*
bool stringToTimestampStruct(const string& data, TimeStamp& dtime)
{
    ...
    uint64_t sec = dataconvert::string_to_ull(data.substr(0, pos), outOfRange);
    // The seconds portion of the timestamp is stored in 44 bits
    // if ( outOfRange || (sec > ((1ULL << 44) - 1)) )
    if ( outOfRange )
    {
        dtime.reset();
        return false;
    ...
    uint64_t msec = dataconvert::string_to_ull(data.substr(pos + 1), outOfRange);
    // The microseconds portion of the timestamp is stored in 20 bits
    // In addition, the last 2 values are used to represent null and
    // empty values
    // if ( outOfRange || (msec > ((1ULL << 20) - 1)) ||
    //      ((sec == ((1ULL << 44) - 1)) && (msec > ((1ULL << 20) - 3))) )
    if ( outOfRange || !isTimestampValid(sec, msec) )
    {
        dtime.reset();
        return false;
    }
